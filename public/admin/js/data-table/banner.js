$(function() {
  ...
  $(function() {
    table = $('#dataTable').DataTable({
      ...
      ajax: window.location.origin + '/dashboard/banner',
    ...
    $('body').on('click', '.create', function(){
      ...
      id = '';
      btn_add = $(this).html(language == "en" ? '<i class="fa fa-refresh fa-spin"></i> Create Banner' : '<i class="fa fa-refresh fa-spin"></i> Tambah Banner');
      $('#modal_cu').modal('show');
      $('#modal_cu .modal-title').text(language == "en" ? "Create Banner" : "Tambah Banner");
      $('#modal_cu #image').hide();
      
      var_url = '/dashboard/banner';
    })
    ...
    $('body').on('click', '.edit', function(){
      ...
      $.ajax({
        url: window.location.origin + '/dashboard/banner/'+ id +'/edit',
        type: 'GET',
        success: function(response){
          $('#modal_cu').modal('show');
          $('#modal_cu .modal-title').text(language == "en" ? "Update Banner" : "Ubah Banner");
          $('#modal_cu #banner_id').val(response.id);
          $('#modal_cu #name').val(response.name);
          $('#modal_cu #image').show();
          $('#modal_cu #image').attr('src', window.location.origin+'/storage/banners/'+response.image);
    ...
    $('#modal_cu').on('hidden.bs.modal', function(){
      btn_add != "" ? btn_add.html(language == "en" ? "Create Banner" : "Tambah Banner") : null;
    ...
    $('body').on('click', '.delete', function(){
      ...
      swal({
        ...
        if (result.value) {
          ...
          $.ajax({
            url: window.location.origin + '/dashboard/banner/' + id,
            type: "DELETE",
            dataType: 'json',
            success: function(response) {
              table.ajax.reload( null, false );
    ...
    $('body').on('click', '.status', function(){
      ...
      swal({
        ...
        if (result.value) {
          ...
          $.ajax({
            url: window.location.origin + '/dashboard/banner/changed/' + id  + '/' + status,
            type: "PATCH",
            dataType: 'json',
            success: function(response) {
              table.ajax.reload( null, false );
