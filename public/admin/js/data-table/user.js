$(function() {
  ...
  $(function() {
    table = $('#dataTableUser').DataTable({
      processing: true,
      serverside: true,
      ajax: window.location.origin + '/dashboard/users',
      columns: [
        {
          data: 'DT_RowIndex',
          name: 'DT_RowIndex',
  ...
  $('body').on('click', '.create', function(){
    ...
    id = '';
    btn_add = $(this).html(language == "en" ? '<i class="fa fa-refresh fa-spin"></i> Create User' : '<i class="fa fa-refresh fa-spin"></i> Tambah Pengguna');
    $('#modal_cu').modal('show');
    $('#modal_cu .modal-title').text(language == "en" ? "Create User" : "Tambah Pengguna");
    $('#modal_cu #show_hide_password').show();
    
    var_url = '/dashboard/users';
  })
  ...
  $('body').on('click', '.edit', function(){
    ...
    $.ajax({
      url: window.location.origin + '/dashboard/users/'+ id +'/edit',
      type: 'GET',
      success: function(response){
        ...
        $('#modal_cu #show_hide_password').hide();
      },
  ...
  $('#modal_cu').on('hidden.bs.modal', function(){
    btn_add != "" ? btn_add.html(language == "en" ? "Create User" : "Tambah Pengguna") : null;
  ...
  $('body').on('click', '.status', function(){
    ...
    swal({
      ...
      if (result.value) {
        ...
        $.ajax({
          url: window.location.origin + '/users/changed/' + id  + '/' + status,
          type: "PATCH",
          dataType: 'json',
          success: function(response) {
            table.ajax.reload( null, false );
  ...
  $('.save_password').click(function(e){
    e.preventDefault();
    $('.save_password').html('<i class="fa fa-spinner fa-spin"></i> Save').attr("disabled", true);
    ...
    $.ajax({
      url: window.location.origin + '/users/changed-password/' + id,
      type: "POST",
      processData: false,
      contentType: false,
      data: data,
      ...
      success: function(response) {
        if(response.errors){
          showSwal('mixin', 'error', response.error);
        }else {
          $('#keyModal').modal('hide');
          table.ajax.reload( null, false );
          showSwal('mixin', 'success', response.success);
        }
        $('.save_password').html('Save').attr("disabled", false);
      },
      error: function (response) {
        if (response.status == 401 || response.status == 419)
        {
