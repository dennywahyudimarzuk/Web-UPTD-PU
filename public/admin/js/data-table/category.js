$(function() {
    ...
    $(function() {
      table = $('#tblCategory').DataTable({
        processing: true,
        serverside: true,
        ajax: window.location.origin + '/dashboard/category',
        columns: [
          {
            data: 'DT_RowIndex',
            name: 'DT_RowIndex',
    ...
    $('body').on('click', '.create', function(){
      ...
      $('#modal_cu').modal('show');
      $('#modal_cu .modal-title').text(language == "en" ? "Create News Category" : "Tambah Kategori Berita");
      
      var_url = '/dashboard/category';
    })
    ...
    $('body').on('click', '.edit', function(){
      ...
      $.ajax({
        url: window.location.origin + '/dashboard/category/'+ id +'/edit',
        type: 'GET',
        success: function(response){
          $('#modal_cu').modal('show');
          $('#modal_cu .modal-title').text(language == "en" ? "Update News Category" : "Ubah Kategori Berita");
    ...
    $('body').on('click', '.delete', function(){
      ...
      swal({
        ...
        if (result.value) {
          ...
          $.ajax({
            url: window.location.origin + '/dashboard/category/' + id,
            type: "DELETE",
            dataType: 'json',
            success: function(response) {
              table.ajax.reload( null, false );
    ...
    $('body').on('click', '.status', function(){
      ...
      swal({
        ...
        if (result.value) {
          ...
          $.ajax({
            url: window.location.origin + '/dashboard/category/changed/' + id  + '/' + status,
            type: "PATCH",
            dataType: 'json',
            success: function(response) {
              table.ajax.reload( null, false );
